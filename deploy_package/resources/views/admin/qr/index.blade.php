                    ...
                    @forelse($tokens as $token)
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                @if($token->is_active)
                                    <a href="{{ route('admin.attendance-qr.image', $token) }}" target="_blank" download="Branch_QR_{{ $token->branch->name }}_{{ $token->token_date->format('Y-m-d') }}.png" class="text-blue-600 hover:text-blue-800 font-medium inline-flex items-center gap-1">
