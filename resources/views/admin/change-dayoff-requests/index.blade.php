<x-layouts.admin>
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Change Dayoff Approval</h2>
            <p class="text-sm text-slate-500 mt-1">Manage employee change dayoff requests</p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200 text-xs uppercase tracking-wider text-slate-500 font-semibold">
                        ...
                        <th class="py-3 px-4">Dayoff Change</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($changeDayoffRequests as $request)
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="py-3 px-4 text-sm text-slate-600 whitespace-nowrap">
                                <span class="bg-slate-100 px-2 py-1 rounded text-slate-500">{{ \Carbon\Carbon::parse($request->old_dayoff)->format('M d, Y') }}</span>
                                <span class="text-slate-400 mx-1">→</span>
                                <span class="bg-blue-50 px-2 py-1 rounded text-blue-600">{{ \Carbon\Carbon::parse($request->new_dayoff)->format('M d, Y') }}</span>
                            </td>
                            ...
                            <td class="py-3 px-4 text-right whitespace-nowrap">
                                @if($request->status === 'pending')
                                    <form method="POST" action="{{ route('admin.change-dayoff-requests.status', $request) }}" class="inline-block mr-1">
                                        @csrf @method('PATCH')
                    ...
                    @empty
                        <tr><td colspan="6" class="py-8 text-center text-slate-500">No change dayoff requests found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if(method_exists($changeDayoffRequests, 'links'))
        <div class="p-4 border-t border-slate-200">
            {{ $changeDayoffRequests->links() }}
        </div>
        @endif
    </div>
    ...
    <div x-data="{ 
            open: false,
            requestId: null,
            baseRoute: '{{ route('admin.change-dayoff-requests.status', '__id__') }}'
         }" 
                ...
                <form method="POST" :action="requestId ? baseRoute.replace('__id__', requestId) : '#'">
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="flex justify-between items-center mb-4 pb-3 border-b border-slate-100">
                            <h3 class="text-lg leading-6 font-semibold text-slate-800" id="modal-title">Reject Change Dayoff Request</h3>
                        ...
                        <div class="mt-2">
                            ...
                            <textarea name="admin_comment" class="w-full border-slate-300 rounded-lg shadow-sm focus:border-red-500 focus:ring focus:ring-red-500 focus:ring-opacity-50 text-sm" rows="3" required placeholder="Please provide a reason for rejecting this change dayoff request..."></textarea>
                        </div>
                    </div>
